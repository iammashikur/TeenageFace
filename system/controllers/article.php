...
*/
    ...
    $the_ip = $this->db2->fetch("SELECT * FROM ip_address WHERE ip_address='".$ip."' ORDER BY id DESC LIMIT 1");
    if($the_ip !== null && $the_ip->ip_address == $ip && $the_ip->code == $D->codearticle && date('Y-m-d H:i:s') < $the_ip->expire_date)
    {
        // View Counter //
    }
    else
    {
        ...
        $this->db2->query("UPDATE articles SET numviews=numviews+1 WHERE code='".$D->codearticle."'");
        $this->db2->query("INSERT INTO ip_address SET ip_address='".$ip."' ,  expire_date='".date('Y-m-d H:i:s', strtotime('+24 hours'))."', code='".$D->codearticle."'");
